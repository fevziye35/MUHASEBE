...
const PurchaseWaybillSelection = () => {
  ...
  const loadCustomers = () => {
    // Alış irsaliyesinde genelde 'Tedarikçi' seçilir, ama şimdilik tüm carileri getiriyoruz.
  ...
  const handleSelectCustomer = (id) => {
    navigate(`/alis-irsaliye-detay?cari_id=${id}`);
  };

  return (
    <div className="dash-container">
      <h2 className="page-title">Alış İrsaliyesi Girişi</h2>
      ...
      <div style={{ marginBottom: '20px' }}>
        <Link to="/cari-yeni" className="btn btn-dark-blue p-20" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', textDecoration: 'none' }}>
          <i className="fa-solid fa-user-plus"></i> Yeni Cari/Tedarikçi Ekle
        </Link>
      </div>
      ...
      <div className="selection-section">
        <div className="section-header bg-gray-header" style={{ padding: '15px 20px', borderRadius: '8px 8px 0 0', display: 'flex', alignItems: 'center', gap: '10px', fontWeight: 'bold' }}>
          ...
          Mal Alınan Cari/Tedarikçi Seçimi
        </div>
                  ...
                  <tr key={c.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                    <td style={{ padding: '12px', textAlign: 'center' }}>
                      <button className="btn btn-secondary" onClick={() => handleSelectCustomer(c.id)}>
                        <i className="fa-solid fa-arrow-right"></i> Seç
                      </button>
                    </td>
...
export default PurchaseWaybillSelection;
